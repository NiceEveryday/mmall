...
public class ProductController {
    ...
    public ResponseContent<ProductDetailVO> detail(Integer productId){
        return  iProductService.detail(productId,true);
    }

    @RequestMapping(value = "list.do",method = RequestMethod.GET)
    @ResponseBody
    public ResponseContent<PageInfo> list(@RequestParam(value = "pageNum" ,defaultValue = "1") Integer  pageNum,
                                           @RequestParam(value = "pageSize",defaultValue = "10") Integer pageSize,
                                           @RequestParam(value = "categoryId",required = false) Integer categoryId,
                                           @RequestParam(value = "keyword",required = false) String keyword,
                                          @RequestParam(value = "orderBy",defaultValue = "price_asc") String orderBy){
        return  iProductService.listFront(categoryId,keyword,pageNum,pageSize,orderBy);
    }
